...
import { Organization } from '@/packages/shared/src';
...
export function DashboardContent({
  ...
  const handleRefresh = async () => {
    try {
      ...
      const newStats = await OrganizationClientService.getOrganizationStats(
        organization.id
      );
      setStats(newStats);
      ...
      toast.error('Failed to refresh data', { id: 'refresh-dashboard' });
      console.error('[DashboardContent] Refresh error:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const quickActions = [
    ...
    },
    {
      ...
      href: `/org/${organization.slug}/bugs`,
      icon: Bug,
    ...
    },
    {
      ...
      href: `/org/${organization.slug}/leaderboard`,
      icon: Trophy,
              ...
              <Link key={action.href} href={action.href}>
                <Card className='border-0 shadow-lg hover:shadow-xl transition-all hover:-translate-y-1 cursor-pointer group'>
                  <CardContent className='p-6'>
                    <div
                      className={`h-12 w-12 rounded-xl bg-gradient-to-br ${action.gradient} flex items-center justify-center mb-4 shadow-md group-hover:scale-110 transition-transform`}
                    >
